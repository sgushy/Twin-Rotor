...
{
    hid_iface_t *interface = NULL;
    ESP_LOGI(TAG, "Get interface for HID protocol %02X", proto);
    HID_ENTER_CRITICAL();
    STAILQ_FOREACH(interface, &ifaces_tailq, tailq_entry) {
        if (proto == interface->proto) {
            HID_EXIT_CRITICAL();
            ESP_LOGI(TAG, "Found interface for HID protocol %02X!", proto);
            return interface;
        }
    }
...
esp_err_t create_interface_new(const hid_host_device_config_t *dev_config,
                                       hid_host_device_handle_t hid_device,
                                       const usb_intf_desc_t *iface_desc,
                                       const hid_descriptor_t *hid_desc,
                                       const usb_ep_desc_t *ep_desc)
{
    ESP_ERROR_CHECK(add_interface_to_list(dev_config, hid_device, iface_desc, hid_desc, ep_desc));
...
//esp_err_t add_new_interface(const )
//{
//
//}
                                   ...
                                   hid_host_interface_handle_t *iface_handle)
{
    ...
    {
        //ESP_LOGI(TAG,"%X:%X:%X:%X:%X:%X:%X:%X:%X:%X",iface->dev_addr,iface->proto,iface->dev.);
    }
    ...
    ESP_LOGI(TAG, "Prepared interface transfer...!");
    ...
    if (report_protocol != HID_REPORT_PROTOCOL_BOOT) {
        ...
        ESP_LOGI(TAG, "Set boot protocol...!");
        report_protocol = HID_REPORT_PROTOCOL_MAX;
        HID_RETURN_ON_ERROR (hid_class_request_get_protocol(iface, &report_protocol) );
        
        ESP_LOGI(TAG, "Verified set boot protocol...!");
...
esp_err_t fs_hid_host_claim_stick_interface(const hid_host_interface_config_t *iface_config,
                                   hid_host_interface_handle_t *iface_handle, uint8_t num)
{
    hid_iface_t *iface = NULL;
    hid_report_protocol_t report_protocol;
    ...
    // Search Interface in list
    //iface->tailq_entry = STAILQ_INSERT_AFTER(&interfaces_tailq, hid_interface, get_interface_by_proto(0x2));//STAILQ_LAST(&interfaces_tailq, hid_interface, );
    iface->dev_addr = 0x1;
    iface->proto = iface_config->proto;
    //iface->dev = STAILQ_LAST(&);
    iface->num = num;
    iface->ep = 0x81;
    iface->ep_mps = 0x0007;
    iface->country_code = 0x00;
    iface->report_size = 0x007A;
    //iface->*report = hid_device->report;
    //iface->*in_xfer = hid_device->ctrl_xfer;
    //iface->user_cb = iface_config->callback;
    //iface->*user_cb_arg = ;
    //iface->ready = true;
    //get_interface_by_proto(iface_config->proto);

    // Interface not found in list
    if (iface == NULL) {
       // ESP_LOGD(TAG,"Could not find device in list, we will create our own!");
       // ESP_LOGD(TAG,"We do a little trolling and hard-code the relevant values!!!");
    //     STAILQ_ENTRY(hid_interface) tailq_entry;
    // uint8_t dev_addr;                       /**< USB device address */
    // hid_protocol_t proto;                   /**< HID Interface protocol */
    // hid_host_device_handle_t dev;           /**< Parent USB HID device */
